...
$(function() {
		window.addEventListener('message', function(event) {
			...
			$('.image').on('click', function () {
				...
				$.post('http://vrp-kr-advanced-shops/putcart', JSON.stringify({item: item, price : price, label : label, count : count, id : id}), function( cb ) {
			...
			$("body").on("click", "#refreshcart", function() {
				$.post('http://vrp-kr-advanced-shops/escape', JSON.stringify({}));
				location.reload(true);
				$('#wrapper').hide("fast");
				$('#payment').hide("fast");
				$('#cart').hide("fast");
				$.post('http://vrp-kr-advanced-shops/refresh', JSON.stringify({}));
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$('#payment').hide("fast");
						$('#cart').hide("fast");
						$.post('http://vrp-kr-advanced-shops/buy', JSON.stringify({Count : value[i].value, Item : value[i].id}));
					} 
					else {
						$.post('http://vrp-kr-advanced-shops/notify', JSON.stringify({msg : "~r~One of the item does not have enough stock or the amount is invalid."}));
					}
				}
			});
			...
			$("body").on("click", "#bossactions", function() {
				$.post('http://vrp-kr-advanced-shops/bossactions', JSON.stringify({}));
				$.post('http://vrp-kr-advanced-shops/escape', JSON.stringify({}));
				location.reload(true);
				$.post('http://vrp-kr-advanced-shops/emptycart', JSON.stringify({}));
				$('#wrapper').hide("fast");
				$('#payment').hide("fast");
				$('#cart').hide("fast");
			});
