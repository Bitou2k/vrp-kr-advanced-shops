...
ASserver = Tunnel.getInterface("vrp_advanced_shop","vrp_advanced_shop")
...
RegisterNUICallback('refresh', function(data, cb)
	...
	ASserver.getOwnedShop({number}, function(data) --ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
		ASserver.getShopItems({number}, function(result) --ESX.TriggerServerCallback('esx_kr_shop:getShopItems', function(result)
...
AddEventHandler('esx_kr_shop:hasEnteredMarker', function(zone)
	if zone == 'center' then
		ShopId = zone
		number = zone
		Msg  = 'Press ~INPUT_CONTEXT~ to open the shopcenter.'--_U('press_to_open_center')
	elseif zone <= 100 then
		ShopId = zone
		number = zone
		Msg  = 'Press ~INPUT_CONTEXT~ to open the shop.' --_U('press_to_open')
	elseif zone >= 100 then
		ShopId = zone
		number = zone
		Msg  = 'Press ~INPUT_CONTEXT~ to ~r~rob ~w~the shop.'--_U('press_to_rob')
	end
end)
			...
			DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				...
				if IsControlJustReleased(0, Keys['E']) then
					...
					elseif ShopId <= 100 then
						--ASserver.getOwnedShop({number}, function(data)
						--	_data = data
						--ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
							ASserver.getOwnedShopAndItems({number})
						--end)
					elseif ShopId >= 100 then
						Robbery(number - 100)
					end
...
function vRPAdvancedShop.returnGetOwnedShopAndItems(data, result)
	if data ~= nil then
		Owner = true
	end
...
local cooldownMenu = 0
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1000)
		if cooldownMenu >= 1 then
			cooldownMenu = cooldownMenu - 1
		end
	end
end)

function vRPAdvancedShop.menuHideBlip()
	showblip = false
	createForSaleBlips()
end
function vRPAdvancedShop.menuShowBlip()
	showblip = true
	createForSaleBlips()
end
...
function OpenBoss(number)

	ASserver.openBossMenu({number})
...
AddEventHandler('esx_kr_shops:setBlip', function()
  	ASserver.getOwnedBlips({})
end)

function vRPAdvancedShop.returnGetOwnedBlips(blips)
...
function Robbery(id)
    ...
    ASserver.getUpdates({id})
end

function vRPAdvancedShop.returnGetUpdates(id,result)
	local coords1 = {
	...
	if result.cb ~= nil then
		if #result.cops >= Config.RequiredPolices then
			...
			TriggerEvent('chatMessage', 'SYSTEM', {255, 0, 0}, "Shop robbery at the " .. result.name .. "\'s shop")
			TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~r~Your store ~b~(" .. result.name .. ')~r~ is under robbery', id)
			
			local safe = GameSpawnObject(1089807209, coords)
			...
			vRP.notify({"~r~There is not enough polices online " .. #result.cops .. '/' .. Config.RequiredPolices})
		end
	else
		vRP.notify({"~r~This shop has already bein robbed, please wait " ..  math.floor((Config.TimeBetweenRobberies - result.time)  / 60) .. ' minutes'})
	end
end


function GameSpawnObject(model, coords)
...
Citizen.CreateThread(function()
	while true do
		...
		elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
			...
			vRP.notify({'~r~The robbery was canceled (you went too far'})
		end
	end
end)
